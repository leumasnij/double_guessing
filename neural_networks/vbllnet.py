...
def save_and_eval(model, folder, device, save_path):
    ...
    for file1 in os.listdir(folder):
        ...
        with torch.no_grad():
            ...
            combined_error = torch.abs(outputs - GT).cpu().numpy()
            combined_error = np.linalg.norm(combined_error, axis=1)
            best_idx = np.argmin(combined_error)
            best_error = (outputs[best_idx] - GT[best_idx]).cpu().numpy()
            # print(f"Average error for {file1}: {average_error}")
            # print(f"Best error for {file1}: {best_error}")
            best_angle = new_dataset[best_idx][-2:].cpu().numpy()
            print(f"Best angle for {file1}: {best_angle}")
            overall_error = np.add(overall_error, average_error)
            average_best_error = np.add(average_best_error, np.abs(best_error))
...
if __name__ == '__main__':
    ...
    root_dir = '/media/okemo/extraHDD31/samueljin/data2'
    save_path = '/media/okemo/extraHDD31/samueljin/Model/MLP2Pos0Pad'
    ...
    val_loader = DataLoader(val_dataset, batch_size=480, shuffle=True, num_workers=4)
    ...
    model.load_state_dict(torch.load('/media/okemo/extraHDD31/samueljin/Model/MLP2Pos0Pad_best_model.pth'))
